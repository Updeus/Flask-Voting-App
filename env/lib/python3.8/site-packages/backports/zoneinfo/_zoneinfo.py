...
class ZoneInfo(tzinfo):
    ...
    def _find_trans(self, dt):
        if dt is None:
            return self._tz_after if self._fixed_offset else _NO_TTINFO
    ...
    def __str__(self):
        return f"{self._key}" if self._key is not None else repr(self)
    ...
    def _unpickle(cls, key, from_cache):
        return cls(key) if from_cache else cls.no_cache(key)
    ...
    def _load_file(self, fobj):
        ...
        for i in range(len(isdst)):
            ...
            self._tti_before = self._ttinfos[0] if self._ttinfos else None
        # Set the "fallback" time zone
        if tz_str is not None and tz_str != b"":
            self._tz_after = _parse_tz_str(tz_str.decode())
        elif self._ttinfos or _ttinfo_list:
            self._tz_after = self._ttinfos[-1] if self._ttinfos else _ttinfo_list[-1]
        else:
...
class _TZStr:
    ...
    def _get_trans_info(self, ts, year, fold):
        ...
        isdst = start <= ts < end if start < end else not (end <= ts < start)
