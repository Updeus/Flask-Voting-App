    ...
    * Abstract models should not have one generated.
    ...
    return next(
        (
            ...
            and isinstance(base, DeclarativeMeta)
...
class NameMetaMixin(type):
    def __init__(self, name, bases, d):
        if should_set_tablename(self):
            self.__tablename__ = camel_to_snake_case(self.__name__)

        super(NameMetaMixin, self).__init__(name, bases, d)

        # __table_cls__ has run at this point
        # if no table was created, use the parent table
        if (
            '__tablename__' not in self.__dict__
            and '__table__' in self.__dict__
            and self.__dict__['__table__'] is None
        ):
            del self.__table__
...
class BindMetaMixin(type):
    def __init__(self, name, bases, d):
        ...
        super(BindMetaMixin, self).__init__(name, bases, d)

        if bind_key is not None and getattr(self, '__table__', None) is not None:
