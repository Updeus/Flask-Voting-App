...
def sqlalchemy_version(op, val):
    ...
    assert op in ('<', '>', '<=', '>=', '=='), f'op {op} not supported'
    ...
    if op == '<=':
        return sa_ver <= target_ver
    return sa_ver >= target_ver if op == '>=' else sa_ver == target_ver


def engine_config_warning(config, version, deprecated_config_key, engine_option):
    if config[deprecated_config_key] is not None:
        warnings.warn(
            f"The `{deprecated_config_key}` config option is deprecated and will be removed in v{version}.  Use `SQLALCHEMY_ENGINE_OPTIONS[\'{engine_option}\']` instead.",
            DeprecationWarning,
        )
